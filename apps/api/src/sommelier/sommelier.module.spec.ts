...
/**
 * T2 — route-level integration that boots ONLY SommelierModule (no AppModule,
 * no Prisma/DB). This proves the full request path — DTO validation, unknown-
 * field stripping, the canned wire shape, the 503 budget kill-switch, and the
 * per-IP throttle — runs green without external services.
 *
 * The companion sommelier.controller.spec.ts / sommelier.throttle.spec.ts boot
 * the whole AppModule (house supertest pattern) and additionally assert the
 * route is wired through the real app graph; those require Postgres (like every
 * other AppModule-based suite) and run in CI.
 */
